...
export const useSetPlace = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      const { placeId, eventId } = variables;

      // 즉시 캐시에 반영
      queryClient.setQueryData<PlaceInfo>(["placeInfo", placeId, eventId], prev => {
        if (!prev) return prev;
        return { ...prev, isConfirmed: true, isChanged: false };
      });

      // 이후 실제 데이터 재확인 (정합성 보장)
      queryClient.invalidateQueries({ queryKey: ["placeInfo", placeId, eventId] });
    },
    onError: error => {
